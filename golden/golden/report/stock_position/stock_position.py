...
def execute(filters=None):
	...
	data = []
	for (company, item,parent_warehouse_rss,parent_section_rss,warehouse) in sorted(iwb_map):
		qty_dict = iwb_map[(company, item, parent_warehouse_rss,parent_section_rss,warehouse)]
		...
		report_data = [item, item_map[item]["item_name"],
			...
			item_map[item]["brand"],
			item_map[item]["description"], parent_warehouse_rss,parent_section_rss,warehouse,
...
def get_conditions(filters):
	...
	if filters.get("section"):
		warehouse_details = frappe.db.get_value("Warehouse", filters.get("section"), ["lft", "rgt"], as_dict=1)
		if warehouse_details:
			conditions += " and exists (select name from `tabWarehouse` wh \
				where wh.lft >= %s and wh.rgt <= %s and sle.warehouse = wh.name)"%(warehouse_details.lft,
				warehouse_details.rgt)
	...
	if filters.get("location"):
		warehouse_details = frappe.db.get_value("Warehouse", filters.get("location"), ["lft", "rgt"], as_dict=1)
		if warehouse_details:
			conditions += " and exists (select name from `tabWarehouse` wh \
				where wh.lft >= %s and wh.rgt <= %s and sle.warehouse = wh.name)"%(warehouse_details.lft,
				warehouse_details.rgt)
...
def get_stock_ledger_entries(filters):
	...
	return frappe.db.sql("""
		select
			sle.item_code, whs.parent_warehouse_rss,whs.parent_section_rss,warehouse, sle.posting_date, sle.actual_qty, sle.valuation_rate,
			sle.company, sle.voucher_type, sle.qty_after_transaction, sle.stock_value_difference
		from
			`tabWarehouse` whs inner join `tabStock Ledger Entry` sle force index (posting_sort_index) %s
		where sle.warehouse = whs.name and sle.docstatus < 2 %s
...
def get_item_warehouse_map(filters):
	...
	for d in sle:
		key = (d.company, d.item_code, d.parent_warehouse_rss,d.parent_section_rss,d.warehouse)
		...
		qty_dict = iwb_map[(d.company, d.item_code, d.parent_warehouse_rss,d.parent_section_rss,d.warehouse)]
...
def filter_items_with_no_transactions(iwb_map):
	for (company, item, parent_warehouse_rss,parent_section_rss,warehouse) in sorted(iwb_map):
		...
		if no_transactions:
			iwb_map.pop((company, item, parent_warehouse_rss,parent_section_rss,warehouse))
