...
frappe.ui.form.on('Payment Entry Receive', {
	onload: function(frm) {
	...
	with_so: function(frm){
		if(frm.doc.with_so == 1){
			...
			while (i--)
			{
					if(tbl[i].reference_doctype == 'Sales Order')
					{
	...
	validate_reference_document: function(frm, row) {
		var _validate = function(i, row) {
			...
			if(frm.doc.party_type=="Customer" &&
				!in_list(["Sales Order", "Sales Invoice"], row.reference_doctype)
...
frappe.ui.form.on('Payment Entry Receive Reference', {
...
frappe.ui.form.on('Payment Entry Receive Deduction', {
	amount: function(frm) {
		frm.events.set_unallocated_amount(frm);
	},
